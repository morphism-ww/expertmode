...
local TRIGGERED_DANGER_MUSIC =
{
    ...
    calamitas_clone = {
        "calamita/calamita/raw_calamity"
    },
...
local function IsInAbyss(player)
    return player.components.areaaware ~= nil
        and player.components.areaaware:CurrentlyInTag("Abyss")
end
...
local function StartDanger(player)
    ...
    elseif _isenabled and not IsInAbyss(player) then
        local x, y, z = player.Transform:GetWorldPosition()
