...
local MinionSpawner = Class(function(self, inst)
    self.inst = inst
    self.miniontype = "lunarthrall_plant"
    self.maxminions = 8
    self.minionspawntime = { min = 15, max = 25 }
    self.minions = {}
    self.numminions = 0
...
function MinionSpawner:OnLostMinion(minion)
    ...
    if self.onlostminionfn~=nil then
        self.onlostminionfn(self.inst,minion)
    end
...
local EYEPLANT_TAGS = { "lunarthrall_plant" }
function MinionSpawner:MakeSpawnLocations()
    ...
    for i = 1, 100 do
        ...
        if ground.Map:IsAboveGroundAtPoint(pos:Get()) and
            ...
            #TheSim:FindEntities(pos.x, pos.y, pos.z, 5, EYEPLANT_TAGS) <= 0 and
