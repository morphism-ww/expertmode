local LinearProjectile = Class(function(self, inst)
    ...
    self.notags = { "INLIMBO", "notarget", "noattack", "flight", "invisible", "playerghost"}
...
function LinearProjectile:AddNoHitTag(tag)
    table.insert(self.notags,tag)
end
...
function LinearProjectile:SetHitDist(d)
    self.hitdist = d
end
...
function LinearProjectile:SetWorker(val)
    self.worker = val
    if val then
        self.musttags = nil
    end
...
function LinearProjectile:CheckForHit()
    ...
    for _,v in ipairs(TheSim:FindEntities(x, 0, z, 3, self.musttags, self.notags,self.oneoftags)) do
        -- The owner/attacker is not a valid target.
        local range = v:GetPhysicsRadius(0.5) + self.hitdist
        if v.entity:IsValid() and v:GetDistanceSqToPoint(x, y, z) < range * range then
            if self.worker and v.components.workable and v.components.workable.workable then
                return true,v
            elseif  attacker.components.combat:CanTarget(v) and
...
function LinearProjectile:OnUpdate(dt)
    ...
    local pos = self.inst:GetPosition()
    local success,target = self:CheckForHit()
    if success then
        self:Hit(target)
