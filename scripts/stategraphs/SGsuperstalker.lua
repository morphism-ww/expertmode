...
local events =
{
    ...
    EventHandler("meteors", function(inst)
        if not (inst.sg:HasStateTag("busy") or inst.components.health:IsDead()) then
            inst.sg:GoToState("meteors")
        end
    end),

    EventHandler("shadowball", function(inst)
        if not (inst.sg:HasStateTag("busy") or inst.components.health:IsDead()) then
            inst.sg:GoToState("shadowball_atk")
        end
    end),
    ...
    EventHandler("attacked", function(inst, data)
        if not inst.components.health:IsDead() then
            ...
            if (not inst.sg:HasStateTag("busy") or inst.sg:HasStateTag("caninterrupt")) and not CommonHandlers.HitRecoveryDelay(inst, 5) then
...
local states =
{
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.components.timer:StartTimer("echo_cd", 15)
        end,
    ...
    State{
        ...
        timeline =
        {
            ...
            TimeEvent(32 * FRAMES, function(inst)
                inst.components.combat:DoAreaAttack(inst, 5, nil, nil, "darkness", AREAATTACK_EXCLUDETAGS)
            end),
            TimeEvent(33*FRAMES,function (inst)
                inst:SpawnDarkSpikes()
            end),
            TimeEvent(47 * FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stalker/head") end),
        },
    ...
    State{
        ...
        onenter = function(inst)
            ...
            --V2C: don't trigger attack cooldown
            --inst.components.combat:StartAttack()
            --inst:StartAbility("meteors")
            inst.components.timer:StartTimer("meteors_cd",40)
        end,
            ...
            FrameEvent(10,function (inst)
                ...
                if inst.sg.statemem.target ~= nil and inst.sg.statemem.target:IsValid() then
                    ...
                    for i = 1,6 do
                        ...
                        proj.target = inst.sg.statemem.target
...
return StateGraph("SGsuperstalker", states, events, "idle")
