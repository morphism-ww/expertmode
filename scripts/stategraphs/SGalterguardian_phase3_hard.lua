...
local function DoWarning(inst)
    local ix, _, iz = inst.Transform:GetWorldPosition()
    local rangersq = 20*20
    for i, p in ipairs(AllPlayers) do
        ...
        if dsq_to_player < rangersq and not p:HasTag("playerghost") then
            p.components.sanity:DoDelta(80)
...
local events =
{
    ...
    EventHandler("doattack", function(inst, data)
        if not (inst.components.health:IsDead() or inst.sg:HasStateTag("busy"))
            ...
            if inst.caneraser and not inst.components.timer:TimerExists("eraser_cd")
                    and dsq_to_target > RANGED_ATTACK_DSQ then
                DoWarning(inst)
            ...
            elseif not inst.components.timer:TimerExists("summon_cd") and dsq_to_target < SUMMON_DSQ then
                if inst.candoflame and math.random()>0.5 then
                    inst.canflame=true
                end
                inst.sg:GoToState("atk_summon_pre", data.target)
            elseif inst.canholylight and not inst.components.timer:TimerExists("holylight_cd") then
                inst.sg:GoToState("atk_holylight",data.target)
            else
    ...
    EventHandler("attacked", function(inst)
        inst:PushEvent("ontrapschanged")
        if not inst.components.health:IsDead() then
            if inst.hasshield then
                local fx = SpawnPrefab("stalker_shield2")
                fx.entity:SetParent(inst.entity)
                fx.AnimState:SetScale(-2.4, 2.4, 2.4)
                fx.AnimState:SetAddColour(127/255,1,212/255,0.3)
            end
        end
    end),
}
...
local function start_summon_circle(inst)
    ...
    if inst.sg.mem.summon_fx == nil then
        ...
        inst.sg.mem.summon_fx.Transform:SetScale(1.6, 1.6, 1.6)
...
local function do_flame_spawn(inst)
    ...
    for i, p in ipairs(ents) do
        local dsq_to_player = p:GetDistanceSqToPoint(ix, 0, iz)
        if dsq_to_player < TUNING.ALTERGUARDIAN_PHASE3_SUMMONRSQ then
            if p:IsValid() and not p:IsInLimbo() and
			not (p.components.health ~= nil and p.components.health:IsDead()) and
			inst.components.combat:CanTarget(p) then
                target_in_range = true

                local flame = SpawnPrefab("warg_mutated_breath_fx")
                flame:SetFXOwner(inst)

                local px, py, pz = p.Transform:GetWorldPosition()

                local radius = GetRandomMinMax(2,4)
                local angle = (inst:GetAngleToPoint(px, py, pz) + GetRandomMinMax(-90, 90)) * DEGREES
                flame.Transform:SetPosition(
                    px + radius * math.cos(angle),
                    py,
                    pz + radius * -math.sin(angle)
                )
                flame:RestartFX(2, "nofade", p)
            end
        end
    end
...
local function testforfailheal(inst)
    if not inst.fixhits then
        inst.fixhits = 0
    end

    if inst.fixhits == 3 then
...
local function SummonHolyLight(inst,target,num,radius)
    if target~=nil and target:IsValid() then
        ...
        for i=1,num do
            ...
            projectile.Transform:SetPosition(a + radius*math.cos(angle), 0, c - radius* math.sin(angle))
            angle=angle+angle_delta
        end
        SpawnPrefab("alter_light").Transform:SetPosition(a, 0, c)
    end
end
...
local states =
{
    ...
    State{
        ...
        events =
        {
            EventHandler("animover", function(inst)
                ...
                if inst.canflame then
                    inst.sg:GoToState("atk_flame_loop")
                elseif inst.canheal then
    ...
    State{
        ...
        timeline =
        {
            ...
            TimeEvent(74*FRAMES, function(inst)
                inst:DoTraps(
                    12,
                    TUNING.ALTERGUARDIAN_PHASE3_TRAP_MINRANGE + 4,
                    TUNING.ALTERGUARDIAN_PHASE3_TRAP_MAXRANGE + 8
                )
            end),
    ...
    State{
        ...
        timeline =
        {
            ...
            TimeEvent(35*FRAMES, function(inst)
                ...
                -- both clockwise and counterclockwise, to get target positions that produce
                -- an aligned tri-beam, sourced at the boss.
            TimeEvent(35*FRAMES , laser_sound),
        ...
        onexit = function(inst)
            inst.components.timer:StartTimer("eraser_cd",70)
            inst.Transform:SetSixFaced()
            inst.sg:RemoveStateTag("nofreeze")
    ...
    State{
        ...
        timeline =
        {
            ...
            TimeEvent(8*FRAMES, do_flame_spawn),
            TimeEvent(16*FRAMES, do_flame_spawn),
        },
    ...
    State{
        ...
        {
            TimeEvent(1*FRAMES, do_stab_attack),
            TimeEvent(8*FRAMES, function(inst)inst.sg.statemem.past_interrupt = nil end),
            TimeEvent(24*FRAMES, function(inst)inst.sg.statemem.past_interrupt = true end),
        ...
        events =
        {
            EventHandler("animover", function(inst)
                ...
                if inst.sg.mem.summon_loops >= TUNING.ALTERGUARDIAN_PHASE3_SUMMONMAXLOOPS then
                    inst.sg.mem.summon_loops = nil
                    inst.sg:GoToState("atk_summon_pst")
                else
                    inst.sg:GoToState("heal_loop")
