...
local function OnFlungFrom(inst, daywalker)
	inst.Follower:StopFollowing()

	local x,y,z
    if daywalker and daywalker:IsValid() then
	...
	rot = rot * DEGREES
	local speedmult = 0.5 + math.random()
...
local events =
{
	...
	EventHandler("attacked", function(inst)
		if not inst.components.health:IsDead() then
            if inst.sg.currentstate.name == "attached" then
                local owner = inst:GetHost()
                OnFlungFrom(inst,owner)
            elseif not inst.sg:HasStateTag("busy") then
			    inst.sg:GoToState("hit")
            end
		end
	end),
	EventHandler("target_loss", function(inst,data)
		if inst.sg.currentstate.name == "attached" then			
			OnFlungFrom(inst,data.host)
		end
	end),
...
local function TryAttach(inst, target)
	if target ~= nil and target.isplayer and target:IsValid() and inst:IsNear(target, 1.8)
		and not (target.components.newcs_talisman~=nil and target.components.newcs_talisman:TryKillThreat(inst)) then
		inst:AttachTarget(target)
	end
end
...
return StateGraph("abyss_leech", states, events, "idle")
