...
local function destroystuff(inst)
    ...
    local ents = TheSim:FindEntities(x, y, z, 5, nil, TARGET_IGNORE_TAGS, TARGET_TAGS)
    for i, v in ipairs(ents) do
        ...
        if v ~= inst.CASTER and v:IsValid() then
            if v.components.health ~= nil and
                ...
                v.components.combat:GetAttacked(inst, 20, nil, "wind")
                v.components.health:DoFireDamage(10, inst.CASTER,true)
                if v.components.temperature~=nil then
                    v.components.temperature:DoDelta(80)
                end
