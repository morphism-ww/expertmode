...
local function tryspawn(inst)
    if inst.resetruins and #inst.components.objectspawner.objects <= 0 then
        ...
        local target_prefab = inst.spawnprefab[math.random(1,#inst.spawnprefab)]

        local obj = inst.components.objectspawner:SpawnObject(target_prefab)
...
local function MakeAbyssRespawnerInst(obj, onrespawnfn, data)
	return Prefab(obj.."_ruinsrespawner_inst", MakeFn(obj, onrespawnfn, data), nil, { obj, obj.."_spawner" })
end

local function MakeAbyssRespawnerWorldGen(obj, onrespawnfn, data)
...
return {Inst = MakeAbyssRespawnerInst, WorldGen = MakeAbyssRespawnerWorldGen}
