...
local function IsCrazy(self)
	return self.inst.components.health.currenthealth < TUNING.STALKER_ATRIUM_PHASE3_HEALTH
end
...
local function ShouldVortex(self)
	return IsCrazy(self) and next(self.inst._vortexes)==nil
end

local function ShouldShadowBall(self)
	if IsDefensive(self) and not IsCrazy(self) and not self.inst.components.timer:TimerExists("shadowball_cd") then
        self.abilitydata = { crazy = IsCrazy(self) }
        return true
    end
    return false
end
